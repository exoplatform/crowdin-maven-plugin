...
All localization file of a given project must be referenced in the file translations.properties located at the root of the project.
...
It will automatically added in Crowdin during the next synchronization.

=== How to update an existing translation string ?

Updating a string must be done in Crowdin directly.
...
Adding a new language must be done in Crowdin.
Once the language has been activated, its localization files will be pushed in the sources during the next synchronization.
